...
if ( empty( $theselevels ) ) {
	...
	$confirmation_message = '<p>' . sprintf( __( 'Thank you for your membership to %1$s. Your %2$s membership is now active.', 'pmpro' ), get_bloginfo( 'name' ), pmprommpu_join_with_and( $levelnames ) ) . '</p>';
} else {
	$confirmation_message = '<p>' . sprintf( __( 'Thank you for your membership to %1$s. Your %2$s memberships are now active.', 'pmpro' ), get_bloginfo( 'name' ), pmprommpu_join_with_and( $levelnames ) ) . '</p>';
}
...
<?php if ( ! empty( $pmpro_invoice ) && ! empty( $pmpro_invoice->id ) ) { ?>		
	...
	<h3>
		<?php printf( __( 'Invoice #%1$s on %2$s', 'pmpro' ), $pmpro_invoice->code, date_i18n( get_option( 'date_format' ), $pmpro_invoice->timestamp ) ); ?>		
	</h3>
	...
	<?php if ( count( $levelnames ) < 2 ) { ?>
			<li><strong><?php _e( 'Membership Level', 'pmpro' ); ?>:</strong> 
		<?php
		if ( 1 === count( $levelnames ) ) {
			echo $levelnames[0];
		} else {
			_e( 'Pending', 'pmpro' );
		}

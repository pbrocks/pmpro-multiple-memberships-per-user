...
function pmprommpu_is_loaded() {
	// If you can run this, we're loaded.
	return true;
}
...
function pmprommpu_get_groups() {
	...
	$allgroups = $wpdb->get_results(
		"
		SELECT * 
		FROM $wpdb->pmpro_groups 
		ORDER BY id
		"
	);
...
/**
 * pmprommpu_create_group Given a name and a true/false flag about whether it allows multiple selections, create a level group.
 *
 * @param  [type]  $inname      [description]
 * @param  boolean $inallowmult [description]
...
function pmprommpu_set_level_for_group( $levelid, $groupid ) {
	...
	$wpdb->insert(
		$wpdb->pmpro_membership_levels_groups, array(
			'level' => $levelid,
			'group' => $groupid,
		), array( '%d', '%d' )
	);
...
/**
 * pmprommpu_get_levels_and_groups_in_order Return an array of the groups and levels in display order - keys are group ID, and values are their levels, in display order
 *
		...
		foreach ( $grouplist as $curgroup ) {
			...
			if ( count( $order ) > 0 ) {
				...
				foreach ( $order as $level_id ) {
					if ( in_array( $level_id, $levelsingroup ) ) {
						$mylevels[] = $level_id; }
				}
...
/**
 * pmprommpu_override_user_pages Called as a filter by pmpro_pages_custom_template_path to add our path to the search path for user pages.
 *
...
/**
 * pmprommpu_set_group_for_level Given a level ID and new group ID, this function sets the group ID for a level. Returns a success flag (true/false).
 *
...
/**
 * pmprommpu_add_group Called by AJAX to add a group from the admin-side Membership Levels and Groups page. Incoming parms are name and mult (can users sign up for multiple levels in this group - 0/1).
 *
 * @return [type] [description]
 */
function pmprommpu_add_group() {
	...
	$displaynum = $wpdb->get_var(
		"
		SELECT MAX(displayorder) 
		FROM {$wpdb->pmpro_groups}
		"
	);
	if ( ! $displaynum || intval( $displaynum ) < 1 ) {
		$displaynum = 1;
	} else {
		$displaynum = intval( $displaynum );
		$displaynum++; }

	if ( array_key_exists( 'name', $_REQUEST ) ) {
		$allowmult = 0;
		if ( array_key_exists( 'mult', $_REQUEST ) && intval( $_REQUEST['mult'] ) > 0 ) {
			$allowmult = 1; }
		$wpdb->insert(
			$wpdb->pmpro_groups,
			array(
...
/**
 * pmprommpu_edit_group Called by AJAX to edit a group from the admin-side Membership Levels and Groups page. Incoming parms are group (the ID #), name and mult (can users sign up for multiple levels in this group - 0/1).
 *
 * @return [type] [description]
 */
...
/**
 * pmprommpu_get_levels_from_latest_checkout This function returns an array of the successfully-purchased levels from the most recent checkout for the user specified by user_id. If user_id isn't specified, the current user will be used. If there are no successful levels, or no checkout, will return an empty array.
 *
 * @param  [type]  $user_id           [description]
 * @param  string  $statuses_to_check [description]
 * @param  integer $checkout_id       [description]
 * @return [type]                     [description]
 */
function pmprommpu_get_levels_from_latest_checkout( $user_id = null, $statuses_to_check = 'success', $checkout_id = -1 ) {
	...
	if ( $checkoutid < 1 ) {
		...
		if ( empty( $checkoutid ) || intval( $checkoutid ) < 1 ) {
			return $retval; }
	}
...
/**
 * [pmprommpu_join_with_and description]
 *
...
function pmprommpu_join_with_and( $inarray ) {
	...
	if ( ! is_array( $inarray ) || count( $inarray ) < 1 ) {
		return $outstring; }
...
/**
 * pmprommpu_hasMembershipGroup Checks if a user has any membership level within a certain group
 *
...
/**
 * pmprommpu_addMembershipLevel Given a membership level (required) and a user ID (or current user, if empty), add them. If an admin wants to force the addition even if it's illegal, they can set force_add to true. Checks group constraints to make sure it's legal first, then uses pmpro_changeMembershipLevel from PMPro core to do the heavy lifting. If the addition isn't legal, returns false. Returns true on success.
 *

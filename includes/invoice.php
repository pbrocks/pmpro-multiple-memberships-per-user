...
class MemberInvoice extends MemberOrder {
		/**
		 * getLastMemberInvoice Returns an order object that represents one or more orders on an invoice (consolidating amounts, etc) If there isn't a recent invoice, or the last one didn't have any matching statuses, then return false.
		 *
	...
	function getLastMemberInvoice( $user_id = null, $status = 'success' ) {
		...
		// build query
		// pbrx thinks this needs replacing
		...
		foreach ( $idarray as $curid ) {
			...
			if ( $temporder ) {
				if ( $ordercount == 1 ) {
					...
					$this->id = 'Multiple'; }
				if ( $ordercount == 1 ) {
					$this->code = $temporder->code;
				} else {
					$this->code = 'Multiple'; }
				$this->session_id = $temporder->session_id;
				$this->user_id = $temporder->user_id;
				if ( $ordercount == 1 ) {
					$this->membership_id = $temporder->membership_id;
				} else {
					$this->membership_id .= ',' . $temporder->membership_id; }
				if ( $ordercount == 1 ) {
					$this->paypal_token = $temporder->paypal_token;
				} else {
					$this->paypal_token = 'Multiple'; }
				...
				if ( $ordercount == 1 ) {
					$this->subtotal = $temporder->subtotal;
				} else {
					$this->subtotal += $temporder->subtotal; }
				if ( $ordercount == 1 ) {
					$this->tax = $temporder->tax;
				} else {
					$this->tax += $temporder->tax; }
				if ( $ordercount == 1 ) {
					$this->couponamount = $temporder->couponamount;
				} else {
					$this->couponamount += $temporder->couponamount; }
				if ( $ordercount == 1 ) {
					if ( strlen( $temporder->certificate_id ) > 0 ) {
						$this->certificate_id = $temporder->certificate_id;
					} else {
						$this->certificate_id = ''; }
				} elseif ( strlen( $temporder->certificate_id ) > 0 && strlen( $this->certificate_id ) > 0 ) {
					$this->certificate_id = 'Multiple'; }
				if ( $ordercount == 1 ) {
					$this->certificateamount = $temporder->certificateamount;
				} else {
					$this->certificateamount += $temporder->certificateamount; }
				if ( $ordercount == 1 ) {
					$this->total = $temporder->total;
				} else {
					$this->total += $temporder->total; }
				...
				} elseif ( $this->status != $temporder->status ) {
					$this->status = 'Multiple'; }
				$this->gateway = $temporder->gateway;
				$this->gateway_environment = $temporder->gateway_environment;
				if ( $ordercount == 1 ) {
					$this->payment_transaction_id = $temporder->payment_transaction_id;
				} else {
					$this->payment_transaction_id = 'Multiple'; }
				if ( $ordercount == 1 ) {
					$this->subscription_transaction_id = $temporder->subscription_transaction_id;
				} else {
					$this->subscription_transaction_id = 'Multiple'; }
	...
	// pbrx wonders if this should be removed
	/**
	 * getMembershipLevels This function returns an array of the membership levels for this invoice and adds the levels to the invoice. It works similarly to MemberOrder::getMembershipLevel(), but with an array, not a single level.
	 *
